...
from apps.mascota.views import listadousuarios, index, mascota_view, mascota_list, mascota_edit, mascota_delete, \
	MascotaList, MascotaCreate, MascotaUpdate, MascotaDelete, MascotaAPI, PostMascotaAPIView, CrearMascotaAPI, NuevaAPI
...
urlpatterns = [
    ...
    url(r'^api', MascotaAPI.as_view(), name="listado"),
    url(r'^(?P<nombre>[\w-]+)/api_update', PostMascotaAPIView.as_view(), name="upd"),
    url(r'^(?P<pk>[\w-]+)/up', CrearMascotaAPI.as_view(), name="i"),
    url(r'^nuevapi', NuevaAPI.as_view(), name="i"),
]
